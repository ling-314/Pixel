...
void Pixel::Renderer::RenderRectangle(Point position, Vector2 offset, D2D1::Matrix3x2F transform, int width, int height, Color color, bool fill, float strokeWidth)
{
	...
	if (fill)
		pRenderTarget->FillRectangle(
			D2D1::RectF(
				(float)position.x - (float)width / 2 - offset.x,
				(float)position.y - (float)height / 2 - offset.y,
				(float)position.x + (float)width / 2 - offset.x,
				(float)position.y + (float)height / 2 - offset.y
			),
			pSolidColorBrush
...
void Pixel::Renderer::RenderImage(Point position, Vector2 offset, D2D1::Matrix3x2F transform, ID2D1Bitmap* pBitmap)
{
	...
	pRenderTarget->DrawBitmap(
		pBitmap,
		D2D1::RectF(
			(float)position.x - pBitmap->GetSize().width / 2 - offset.x,
			(float)position.y - pBitmap->GetSize().height / 2 - offset.y,
			(float)position.x + pBitmap->GetSize().width / 2 - offset.x,
			(float)position.y + pBitmap->GetSize().height / 2 - offset.y
		),
		1.0f,
...
{	
	...
	pRenderTarget->DrawTextLayout(
		D2D1::Point2F(
			(float)position.x - metrics.width / 2 - offset.x,
			(float)position.y - metrics.height / 2 - offset.y
		),
		pTextLayout,
		pSolidColorBrush
	);
