...
void Pixel::Text::Create()
{
	...
	if (FAILED(hr)) SafeRelease(pFactory);
	...
	size(Point(position.x - metrics.height / 2, position.y - metrics.width / 2), Point(position.x + metrics.height / 2, position.y + metrics.width / 2));
...
void Pixel::Text::Render(Renderer* renderer)
{
	DWRITE_TEXT_METRICS metrics;
	pTextLayout->GetMetrics(&metrics);
	renderer->RenderText(position, color, pTextLayout, metrics.height);
}
